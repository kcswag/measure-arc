...
while True:
	...
	#red point
	# if redCntsLen > 0:
	# 	d = max(redCnts, key=cv2.contourArea)
	# 	((centroidX, centroidY), redRadius) = cv2.minEnclosingCircle(d)
	#
	# 	N = cv2.moments(d)
	# 	if N["m00"] == 0:
	# 		continue
	# 	redCenter = (int(N["m10"] / N["m00"]), int(N["m01"] / N["m00"]))
	#
	# 	position2 = (redCenter[0] - 70, redCenter[1] + 70)
	# 	cv2.putText(frame, "(" + str(redCenter[0]) + "," + str(redCenter[1]) + ")", position2, font, 1, (0, 0, 255))
	#
	# 	if redRadius > 5:
	# 		cv2.circle(frame, (int(centroidX), int(centroidY)), int(redRadius), (0, 255, 255), 2)
